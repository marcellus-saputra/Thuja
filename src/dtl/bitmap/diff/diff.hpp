...
public:
  ...
  /*
    update_res:
    0 -> Actual value same as val -> update cancelled
    1 -> In-place update
    2 -> Diff update
    update_type:
    0 -> diff update
    1 -> hybrid + 2-run update
    2 -> hybrid update
    3 -> in-place update
  */
  int
  update(std::size_t i, u1 val, u1 two_run_opt = false) {
    u1 diff_val = diff_->test(i);
    int update_res = bitmap_->update(i, val, &diff_val, two_run_opt);

    // actual value same as val -> skip this update
    if (update_res == 0){
      return 0;
    }

    // cancelled run-breaking update -> diff update
    // update_res != 0 implies actual value == !val -> flip diff_val
    if (update_res == 2) {
      diff_->set(i, !diff_val);
      diff_->shrink();
      return 2;
    }
