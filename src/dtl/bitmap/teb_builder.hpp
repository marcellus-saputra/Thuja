...
/* Note from Marcellus: here I just removed the 'magic numbers' so that the rank and level offset lookup tables are always generated.
Previously they are only generated when the number of bits (uncompressed) is more than 1024, so it just makes testing with smaller
TEBs easier. I'd also bet most real databases have more than 1024 rows, so it shouldn't make a difference. */
...
teb_builder::serialize(word_type* dst) {
  ...
  hdr.trailing_inner_node_cnt = static_cast<u32>(bitmap_tree_.get_trailing_leaf_node_cnt());
  ...
  hdr.implicit_trailing_label_cnt = static_cast<u32>(bitmap_tree_.get_trailing_0label_cnt());
  ...
  hdr.has_level_offsets = u8(1);
  hdr.update_counter = 0;
  hdr.update_threshold = 10;
  ...
  *p = hdr;
  ...
  if (hdr.has_level_offsets) {
    // Level offset LuT.
    //const auto entry_cnt = hdr.encoded_tree_height - hdr.perfect_level_cnt;
    const auto entry_cnt = hdr.encoded_tree_height;
    ...
    std::size_t u = 1;
    for (std::size_t i = 0; i < hdr.perfect_level_cnt; ++i) {
      ofs_tree[i] = u - 1;
      u *= 2;
      ofs_labels[i] = 0;
    }
    for (std::size_t i = hdr.perfect_level_cnt; i < entry_cnt; ++i) {
      ofs_tree[i] = static_cast<size_type>(level_offsets_tree[i]);
      ofs_labels[i] = static_cast<size_type>(level_offsets_labels[i]);
    }
  }
}
//===----------------------------------------------------------------------===//
