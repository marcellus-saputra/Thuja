...
benchmark_result perform_benchmark(benchmark_task benchmark, Update_Type type, std::ofstream &log_file, std::ofstream &res_file) {
    ...
    clock_t diff_update_merge_total_t = 0;
    ...
    for (update_task update : benchmark.run_breaking) {
        ...
        int update_res = new_teb.teb_->update(update.pos, update.val);
        ...
        if (update_res == 1) {
            ...
            if (update_count % 10000 == 0) {
                ...
                diff_update_merge_total_t += merge_t;
                merge_count++;
            }

            update_total_t += update_t;
            diff_update_merge_total_t += diff_update_t;
    ...
    log_file << "Average in-place update time: " << (float) update_total_t / CLOCKS_PER_SEC / update_count * 1000000 << " micros\n";
    ...
    log_file << "Average update time diff with merging: " << (float) diff_update_merge_total_t / CLOCKS_PER_SEC / update_count * 1000000 << " micros\n";
    log_file << "Times merged: " << merge_count << "\n";
    log_file << "Average update time without merging: " << (float) diff_update_total_t / CLOCKS_PER_SEC / update_count * 1000000 << " micros\n\n";
...
int main() {
    ...
    prune_test_log.open("runbreaking_v_diff_log.txt");
    std::vector<benchmark_task> benchmark_list;

    std::ofstream res_file;
    res_file.open("runbreaking_v_diff_res.txt");
